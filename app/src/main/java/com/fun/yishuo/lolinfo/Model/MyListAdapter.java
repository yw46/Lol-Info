...
public class MyListAdapter extends ArrayAdapter<String> {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            viewHolder.title = (TextView) convertView.findViewById(R.id.listChampName);
            viewHolder.thumbnail = (ImageView) convertView.findViewById(R.id.listChampThumbnail);
            convertView.setTag(viewHolder);
        }
        mainViewHolder = (ViewHolder) convertView.getTag();

        String tmpName = getItem(position);
        mainViewHolder.title.setText(tmpName);
        //mainViewHolder.thumbnail.setImageResource(getDrawable(tmpName));
        mainViewHolder.thumbnail.setImageResource(GlobalFunction.getDrawable(tmpName));
        return convertView;
    }

    /*
    private int getDrawable(String champ) {
        if (champ.charAt(0) == 'A') {
            if (champ.charAt(1) == 'a') {
                ...
                return R.drawable.champ_aurelion_sol_square;
